...
def run_model_tests(my_predbat, prediction_kernel=False):
    ...
    # Solar diversion blocked when exporting the surplus pays more than the cheap charge it would displace.
    # Same 1kW PV as car_solar_divert, but the export rate (5) is above the threshold, so nothing is diverted
    # and the PV charges the home battery instead - exactly the car_solar_off outcome.
    failed |= simple_scenario(
        ...
        car_solar_max_power=1.0,
        car_solar_limit=100,
        car_solar_export_threshold=export_rate - 1,
        assert_final_car_solar=0,
    )
    # ...and taken again as soon as the export rate is at or below the threshold
    failed |= simple_scenario(
        "car_solar_export_allowed",
        my_predbat,
        0,
        1.0,
        assert_final_metric=0,
        ...
        car_solar_max_power=1.0,
        car_solar_limit=100,
        car_solar_export_threshold=export_rate,
        assert_final_car_solar=24,
    )
